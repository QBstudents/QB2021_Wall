...
rm(list=ls()) 
getwd() 
setwd("~/Classwork/Current/QB/2.Worksheets/7.DataWrangling")
```
...
```{r}
options(warn=-1)
# simulate big old community
my_comm <- sim_poisson_community(s_pool=200, n_sim=10000, sad_type = "lnorm", 
      sad_coef = list("meanlog" = 2, "sdlog" = 2))

# sample from the community with quadrats at different area sizes
quad_areas <-  seq(0.005, 0.5, 0.005)
dfs <- vector("list", length(quad_areas))
for (qa in 1:length(quad_areas)) {
  samps <- sample_quadrats(my_comm, 5, quadrat_area = quad_areas[qa], plot = FALSE, avoid_overlap=F)
  samp_df <- samps$spec_dat %>%
    mutate(richness = rowSums(. > 0)) %>%
    mutate(area = quad_areas[qa]) %>%
    select(area, richness)
  dfs[[qa]] <- samp_df
}
all_samps_nonzero <- bind_rows(dfs) %>%
  filter(richness > 0) %>%
  mutate(ln_richness = log(richness)) %>%
  mutate(ln_area = log(area))
SA_plot <- ggplot(all_samps_nonzero, aes(x = ln_area, y = ln_richness)) +
  geom_point() +
  stat_smooth(method='lm')
SA_plot
linreg <- lm(ln_richness ~ ln_area, all_samps_nonzero)
print('slope of regression line:')
print(linreg$coefficients[['ln_area']])
```
...
options(warn=-1)
# simulate big old community
my_comm <- sim_thomas_community(s_pool=200, n_sim=10000, sad_type = "lnorm",
        sad_coef = list("meanlog" = 2, "sdlog" = 2))
plottable_comm <- sim_thomas_community(s_pool=20, n_sim=1000)
plot(plottable_comm)
...
all_samps_nonzero <- bind_rows(dfs) %>%
  filter(richness > 0) %>%
  mutate(ln_richness = log10(richness)) %>%
  mutate(ln_area = log10(area))
SA_plot <- ggplot(all_samps_nonzero, aes(x = ln_area, y = ln_richness)) +
  geom_point() +
  stat_smooth(method='lm')
SA_plot
...
> For the poisson community, we are fitting a line to something that doesn't really look like a line. This means that the power law form of our species area relationship does not hold for this kind of spatially homogenous community. No matter where we sample we would expect to see the same abundances of species as area is increased. This will be completely dictated by the rank abundance distribution. We end up with something with an upper asympytote.
>
> When we use the clumpy spatial simulation, we see a more linear curve although it is still saturating. This probably reflects the fact that different locations are not perfect statistical samples of the entire space because they are likely to be enriched for particular species. This is more realistic than the spatially homogenous simulation above. The improved linear fit suggests that the power law form is more appropriate in this case.
> 
> In both cases, our $z$ values are quite high. The slope of the curve is dependent on the species abundance distribution used to generate the community and the associated parameters. In order to improve realism we could use empirical SAD estimates in generating our communities. We could also consider larger scale spatial structure such as correlations between clumps of species that might indicate community structure within a larger metacommunity.
